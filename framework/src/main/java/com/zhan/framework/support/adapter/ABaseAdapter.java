...
public abstract class ABaseAdapter<T> extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(convertView != null){
            ...
            typeChanged=itemViewProcessor.getItemViewType()!=getItemViewType(position);
        }

        if (convertView == null || !isReusing||typeChanged) {
            itemViewProcessor = newItemView();
            itemViewProcessor.position = position;
            itemViewProcessor.size = datas.size();
            itemViewProcessor.viewTypeCount=getViewTypeCount();
            itemViewProcessor.itemViewType=getItemViewType(position);

            convertView=itemViewProcessor.createConvertView(position);
            if(convertView==null){
